...
\documentclass
	[
		 ...
		 parskip=half, %Absaetze durch halben Zeilenabstand statt Einzug trennen.
		               %Gilt dokumentweit - NIEMALS zusaetzlich \medskip o.ae. im Text setzen,
		               %sonst wird der Abstand wieder uneinheitlich.
...
%	\usepackage[abs]{overpic}
	...
	% Einheiten:
	% Das Paket siunitx stellt \qty{Zahl}{Einheit}, \qtyrange{}{}{}, \num{} und \unit{} bereit.
	% Achtung: NICHT zusammen mit dem Paket "units" laden, beide definieren \unit mit
	% unterschiedlicher Syntax (units: \unit[1064]{nm} vs. siunitx: \qty{1064}{\nano\metre}).
	\usepackage{siunitx}
	\sisetup{
		locale                = DE,     % deutsche Konventionen, u. a. Dezimalkomma
		input-decimal-markers = {.,},   % Punkt und Komma als Eingabe erlaubt
		range-phrase          = { bis },
		range-units           = single, % "3 bis 4 eV" statt "3 eV bis 4 eV"
		per-mode              = symbol,
	}
	% Nicht in siunitx vordefinierte Einheiten.
	% \bar kollidiert mit dem Mathe-Akzent \bar und muss deshalb selbst deklariert werden.
	\DeclareSIUnit{\zoll}{Zoll}
	\DeclareSIUnit\bar{bar}
	\DeclareSIUnit{\stk}{Stk.}
	\DeclareSIUnit{\zyklus}{Zyklus}
	...
	\usepackage{float}
	% Auf reinen Float-Seiten (Seiten, die nur Abbildungen/Tabellen enthalten)
	% verteilt LaTeX den freien Platz per Default oben UND unten, der Float
	% steht dadurch mittig. \@fptop auf 0pt setzt ihn an den Seitenanfang;
	% der verbleibende Platz geht dann komplett nach unten (\@fpbot).
	\makeatletter
	\setlength{\@fptop}{0pt}
	...
	%Farben: muss vor tikz und vor den RWTH-Farbdefinitionen geladen werden.
	\usepackage{xcolor}

	%für Grafiken direkt in Latex:
	\usepackage{tikz}
   	\usetikzlibrary{positioning, arrows.meta, fit, decorations.pathmorphing, backgrounds, calc, patterns}
	\usepackage[european]{circuitikz}
	\ctikzset{tripoles/mos style/arrows}

	%RWTH Corporate-Design-Farben (offizielle Werte, siehe styles/rwthcolors.tex)
	\input{styles/rwthcolors}

	%Einheitliche Bedeutungsebene fuer alle selbst erzeugten Abbildungen.
	%Muss NACH rwthcolors und nach tikz geladen werden.
	\input{styles/abbildungsstil}
...
%	\captionsetup[table]{labelfont={bf,small}, font={bf,small}}
	...
	%Transparent (xcolor ist bereits oben geladen)
	...
	%xltabular = longtable + tabularx: X-Spalte UND Seitenumbruch. Wird fuer
	%das Abkuerzungsverzeichnis gebraucht, das laenger als eine Seite ist.
...
\input{glos/Formelzeichen}
\newpage

%Abbildungsverzeichnis
\listoffigures

Alle Abbildungen ohne Quellenangabe sind eigene Darstellungen beziehungsweise eigene Aufnahmen.
\newpage
...
\clearpage
\input{docs/Grundlagen}
\clearpage
\input{docs/StandDerTechnik}
\clearpage
\input{docs/Konzept}
\clearpage
\input{docs/Konstruktion}
\clearpage
\input{docs/Validierung_Tests}
\clearpage
\input{docs/Diskussion}
\clearpage
